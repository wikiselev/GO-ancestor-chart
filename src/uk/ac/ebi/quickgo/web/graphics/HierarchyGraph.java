...
public class HierarchyGraph {
    ...
    public static void main(String[] args) throws Exception {
      ...
      System.out.println("HierarchyGraph - parameters can be:\n\t'--terms=fileName' => fileName contains the list of GO ids or color code, one per line."
          + "\n\t'--name=outputImageName' => prefix that will be used to create the output file names.\n\t'GO:0051674 #0080FF GO:0048870' => you can also specify GO ids directly"
          + "as parameters separated by space. If you add a color code all the GO ids following will be colored with it.\n");
        ...
        if (!"".equals(termList)) {
            try {
          			...
          			while ((goID = reader.readLine()) != null) {
          	            ...
          	            else if (goID.startsWith("#")) {
                          int[] rgb=ColourUtils.decodeColour(goID);
        ...
        for (Term.Ontology o : terms.keySet()) {
            ...
            // Saving a svg file as well - example taken from http://xmlgraphics.apache.org/batik/using/svg-generator.html
            SVGGraphics2D svgGenerator = image.renderSvg();
            boolean useCSS = true; // we want to use CSS style attributes
            Writer out = new FileWriter(outputName + o.text + ".svg");
